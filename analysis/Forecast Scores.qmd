...
observations %>%
  ...
  #geom_line(aes(group = paste0(year, site_id)), data = . %>% filter(year == 2024), size = .8)+
  ggh4x::facet_nested(.~Temp, labeller = label_parsed)+
  #facet_wrap(~site_id)+
  theme_bw() +
  scale_x_date(date_breaks = "4 month", date_labels = "%b")+
  scale_y_continuous(n.breaks = 3)+
...
calculate_pe(test %>% arrange(datetime) %>% pull(observation), m = 3, tau = 6)
calculate_pe(sin(1:1000), m = 2*pi)
calculate_pe(sin(1:2400*(2*pi)/12), m = 12)
...
observations %>%
  ...
  summarize(mean = mean(observation, na.rm = T),
            ...
            cv = sd/mean,
            WPE = calculate_pe(observation, m = 12, tau = 6)[1],
