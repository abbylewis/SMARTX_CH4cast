...
ensemble <- ncvar_get(ncFile_t,"number")
horizon <- ncvar_get(ncFile_t,"forecastMonth")
fillvalue <- ncatt_get(ncFile_t, "t2m","missval") 

ids <- expand.grid(Lon, Lat, reference_datetime, horizon, ensemble)
colnames(ids) <- c("Lon", "Lat", "reference_datetime", "horizon", "ensemble")
...
df <- data.frame(ids, 
                 ...
                 Soil_Temp_K = as.numeric(soil_temp_k), 
                 Solar_Rad_Wm2 = as.numeric(solar_rad_wm2), 
                 Precip_Rate_Ms = as.numeric(precip_rate_ms)) %>%
...
#https://weatherspark.com/compare/y/21929~21008~21691/Comparison-of-the-Average-Weather-in-Centreville-Silver-Spring-and-Edgewater
  pivot_longer(cols = c(Temp_K, Pressure_Pa, Soil_Temp_K, Solar_Rad_Wm2, Precip_Rate_Ms),
               names_to = "variable",
