...
forecast_model <- function(site,
  ...
  if(!var %in% names(site_target) || sum(!is.na(site_target[var])) == 0){
    ...
    matrix <- as.matrix(site_target %>% select(Temp_K, SolarRad_Wm2, PrecipRate_ms, WindSpeed_ms, Pressure_Pa))
    cv <- xgboost::xgb.cv(label = site_target[[var]],
                           data = matrix,
                           nfold = 10,
                           nrounds = 100,
    ...
    fit <- xgboost(data = matrix,
            label = site_target[[var]],
            nrounds = nrounds)
    pred <- predict(fit, matrix)
    plot(site_target[[var]])
    lines(pred)
    ...
    forecast <- noaa_future_monthly |> 
      ...
      summarise(mu = mean(prediction, na.rm = T),
                sigma = sqrt(sd(prediction, na.rm = T)^2 + 
                               sd(pred - site_target[[var]], na.rm = T)^2),
                .groups = "drop") %>%
