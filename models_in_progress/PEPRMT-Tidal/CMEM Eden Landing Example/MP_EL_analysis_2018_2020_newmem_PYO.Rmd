---
title: "Analysis of 2018-2020 Eden Landing data using PEPRMT-Tidal Model"
author: "Michael Najarro, Patty Oikawa"
date: "11/18/2020, updated June 2023"
...
elevation<- read.table(file="EDENLAND082620.txt",sep=",")
...
#2.d) Calculate Annual NPP.
# latest units used:
# (g OM/cm^2*year) * 100^2 cm^2/M^2 = g OM/m^2*year
#multiply OM by 0.5 to get gCm-2 y-1; multiply BGB by 0.5 as you assume 50% turns over every yr
an_NPP <- (annual_agb + (0.5*annual_bgb))*10000*0.5 
an_NPP_b<- ( (0.5*annual_bgb))*10000*0.5 

#Or another way to estimate NPP is to take AGB+BGB-Decomposed OM
an_biomass <- (annual_agb + (annual_bgb))*10000*0.5 
#an_NPP_2 <-an_biomass-
...
percentOM <- 0.5
  ...
  #Calculate for each layer and get a SD across layers of SOM
  fast_OM_20_by_layer <- ((cohort20$fast_OM*percentOM)/(cohort20$layer_bottom-cohort20$layer_top))#g C cm3
  slow_OM_20_by_layer<- ((cohort20$slow_OM*percentOM)/(cohort20$layer_bottom-cohort20$layer_top))#g C m3
  Total_OM_20_by_layer<-slow_OM_20_by_layer+fast_OM_20_by_layer
  plot(Total_OM_20_by_layer)
  ave_OM_20_by_layer<-mean(Total_OM_20_by_layer)
 SD_OM_20_by_layer<-sd(Total_OM_20_by_layer)
 min_OM_20_by_layer<-min(Total_OM_20_by_layer)
  max_OM_20_by_layer<-max(Total_OM_20_by_layer)
  #remove initial low OM layers at spin up
  boxplot(Total_OM_20_by_layer)
  
  #Look at whole core
  fast_OM_by_layer <- ((cohort$fast_OM*percentOM)/(cohort$layer_bottom-cohort$layer_top))#g C cm3
  slow_OM_by_layer<- ((cohort$slow_OM*percentOM)/(cohort$layer_bottom-cohort$layer_top))#g C m3
  Total_OM_by_layer<-slow_OM_by_layer+fast_OM_by_layer
  plot(Total_OM_by_layer)
  ave_OM_by_layer<-mean(Total_OM_by_layer)
 SD_OM_by_layer<-sd(Total_OM_by_layer)
 min_OM_by_layer<-min(Total_OM_by_layer)
  max_OM_by_layer<-max(Total_OM_by_layer)
  #remove initial low OM layers at spin up
  boxplot(Total_OM_by_layer)

  
  #For comparison with Ari's paper, only look at top 30cm of core
  ave_OM_20_top_30<-mean(Total_OM_by_layer[3070:3100])
 SD_OM_20_top_30<-sd(Total_OM_by_layer[3070:3100])
 min_OM_20_top_30<-min(Total_OM_by_layer[3070:3100])
  max_OM_20_top_30<-max(Total_OM_by_layer[3070:3100])
  
  #Sediment accretion check
sediment_accretion20 <-cohort20$layer_bottom-cohort20$layer_top
sediment_accretion <-cohort$layer_bottom-cohort$layer_top
mean(sediment_accretion20)
mean(sediment_accretion)
plot(sediment_accretion)
plot(cohort$cumCohortVol)
...
```{r message=FALSE}
data <- read_csv(file="EL_2018_2020_master_9_30_20_EVI_uncertainty.csv") %>%
...
#3.b) run the loop
  #for(i in 1:length(year_vector)) {
   ...
   LUE<-0.65
    ...
    GPP_mod_EL <- PEPRMT_GPP_final(theta, data)
...
#3.b) run the loop
  for(i in 1:length(year_vector)) {
    ...
    #added Dec 2021
    percentOM <- 0.5
  cum_vol_cohorts_16<- last(cohort_i$cumCohortVol)#should be in cm-3
      theta5 <- ((sum(cohort_i$respired_OM)*percentOM)/cum_vol_cohorts_16)*1e6#g C m3
...
# 2.a) tack on GPP  and Reco results onto end
#data[5] <-  c(rep(0, nrow(data)))
data[21] <- GPP_mod_EL
data[22:25] <- ER_mod_RR


#PLOT RESULTS  
    plot(data$Day_of_year,data$Reco_full, type="b", pch=19, col="red", xlab="DOY", ylab="Reco")
# Add a line
lines(data$Day_of_year, data$Reco_gC_m2_day, pch=18, col="blue", type="b", lty=2)
# Add a legend
legend(100, 2, legend=c("mod", "obs"),
       #lty = c(1,2),
       col=c("red", "blue"))#, lty=1:2, cex=0.8)
...
sulfate <- 87
nitrate <- 0.8
theta <- c(15, 0, 15, 0, 15, 0, sulfate, nitrate)
